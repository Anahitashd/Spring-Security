...
public class CaptureRuleCrudServiceImpl extends GlobalCommonServiceImpl<CaptureRule> implements CaptureRuleCrudService {
    ...
    private void validateUserAccessToGroupedConnections(
        ...
        boolean hasCurrentUserGroup = StringUtils.hasContent(currentUserGroupName);
        JpaQuery allGroupedConnectionsOfAllCaptureRulesAssignedToUserQuery = new JpaQueryBuilder()
                .select("cr.name", "c.name")
                .from(User.class, "u")
                .distinct()
                .join("captureRules", "cr")
        ...
        for (Object item : this.jpaQueryBasedReadRepository
                .findAll(allGroupedConnectionsOfAllCaptureRulesAssignedToUserQuery,
                        tuple -> new String[]{(String) tuple.get(0), (String) tuple.get(1), null})) {

            String[] names = (String[]) item;

            for (String[] connectionName : connectionsToCheckAgainst) {
                if (names[1].equalsIgnoreCase(connectionName[0])) {
                    if (connectionName.length == 2) {
                        throw hasCurrentUserGroup
                                ? new UserGroupUserAlreadyAccessConnectionByAnotherCaptureRuleException(
                                username, names[0], names[1], connectionName[1], currentUserGroupName
                        )
                                : new UserAlreadyAccessConnectionByAnotherCaptureRuleException(username, names[0], names[1], connectionName[1]);
                    }

                    throw hasCurrentUserGroup
                            ? new UserGroupUserAlreadyAccessConnectionByAnotherCaptureRuleException(username, names[0], names[1], currentUserGroupName)
                            : new UserAlreadyAccessConnectionByAnotherCaptureRuleException(username, names[0], names[1]);
                }
            }
        }
        ...
        for (Object item : this.jpaQueryBasedReadRepository
                .findAll(
                        allIndividualConnectionsOfAllCaptureRulesAssignedToUserThroughGroupsQuery,
                        tuple -> new String[]{(String) tuple.get(0), (String) tuple.get(1), (String) tuple.get(2), null}
                )) {
            String[] names = (String[]) item;
            for (String[] connectionNamex : connectionsToCheckAgainst) {
                if (names[1].equalsIgnoreCase(connectionNamex[0])) {
                    if (connectionNamex.length == 2) {
                        throw hasCurrentUserGroup
                                ? new UserGroupUserAlreadyAccessConnectionByAnotherCaptureRuleThroughAnotherUserGroupException(
                                username, names[0], names[1], connectionNamex[1], names[2], currentUserGroupName
                        )
                                : new UserAlreadyAccessConnectionByAnotherCaptureRuleThroughUserGroupException(username, names[0], names[1], connectionNamex[1], names[2]);
                    }

                    throw hasCurrentUserGroup
                            ? new UserGroupUserAlreadyAccessConnectionByAnotherCaptureRuleThroughAnotherUserGroupException(
                            username, names[0], names[1], names[2], currentUserGroupName
                    )
                            : new UserAlreadyAccessConnectionByAnotherCaptureRuleThroughUserGroupException(username, names[0], names[1], names[2]);
                }
            }
        }
    }
    ...
    private void validateUserAccessAsIndividual(
            JpaQuery query, List<String[]> connectionsToCheckAgainst, String username, String currentUserGroupName, boolean isUserInAGroup
    ) throws UserAlreadyAccessConnectionByAnotherCaptureRuleException {
        for (Object item : this.jpaQueryBasedReadRepository
                .findAll(query, tuple -> new String[]{(String) tuple.get(0), (String) tuple.get(1)})) {
            String[] names = (String[]) item;
            for (String[] connectionNameToCheckAgainst : connectionsToCheckAgainst) {
                if (names[1].equalsIgnoreCase(connectionNameToCheckAgainst[0])) {
                    if (connectionNameToCheckAgainst.length == 2) {
                        throw isUserInAGroup
                                ? new UserGroupUserAlreadyAccessConnectionByAnotherCaptureRuleException(
                                username, names[0], names[1], connectionNameToCheckAgainst[1], currentUserGroupName
                        )
                                : new UserAlreadyAccessConnectionByAnotherCaptureRuleException(username, names[0], names[1], connectionNameToCheckAgainst[1]);
                    }
    ...
    private void validateUserAccessAsGrouped(
        ...
        for (Object item : this.jpaQueryBasedReadRepository
                .findAll(query, tuple -> new String[]{(String) tuple.get(0), (String) tuple.get(1), (String) tuple.get(2)})) {
            String[] names = (String[]) item;
            for (String[] connectionName : connectionsToCheckAgainst) {
                if (names[1].equalsIgnoreCase(connectionName[0])) {
                    if (connectionName.length == 2) {
                        throw isUserInAGroup
                                ? new UserGroupUserAlreadyAccessConnectionByAnotherCaptureRuleThroughAnotherUserGroupException(
                                username, names[0], names[1], connectionName[1], names[2], currentUserGroupName
                        )
                                : new UserAlreadyAccessConnectionByAnotherCaptureRuleThroughUserGroupException(username, names[0], names[1], connectionName[1], names[2]);
                    }
    ...
    private void validateUserGroupAccess(JpaQuery query, List<String[]> connectionToCheckAgainst, String userGroupName) throws UserGroupAlreadyAccessConnectionByAnotherCaptureRuleException {
        for (Object item : this.jpaQueryBasedReadRepository
                .findAll(query, tuple -> new String[]{(String) tuple.get(0), (String) tuple.get(1)})) {
            String[] names = (String[]) item;
            for (String[] connectionName : connectionToCheckAgainst) {
                if (names[1].equalsIgnoreCase(connectionName[0])) {
                    if (connectionName.length == 2) {
                        throw new UserGroupAlreadyAccessConnectionByAnotherCaptureRuleException(userGroupName, names[0], names[1], connectionName[1]);
                    }

                    throw new UserGroupAlreadyAccessConnectionByAnotherCaptureRuleException(userGroupName, names[0], names[1]);
                }
            }
        }
    }
