...
public class InputImageCfillExtractor implements InputExtractor<Void> {
   @Override
   public boolean extractable(GuacamoleInstruction instruction, RemoteSessionInputExtraction remoteSessionInputExtraction) {
      return remoteSessionInputExtraction.getInputStatus().equals(ConsoleTypeRemoteSessionInputProcessingStatus.IMAGE_RECT)
         && instruction.getOpcode().equalsIgnoreCase("cfill");
   }
