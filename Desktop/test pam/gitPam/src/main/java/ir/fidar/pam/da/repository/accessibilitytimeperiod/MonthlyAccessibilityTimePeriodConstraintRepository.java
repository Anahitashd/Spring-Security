...
public interface MonthlyAccessibilityTimePeriodConstraintRepository extends JpaRepository<MonthlyAccessibilityTimePeriodConstraint, Long> {
   ...
   @Query("SELECT mc FROM MonthlyAccessibilityTimePeriodConstraint mc " +
           "WHERE mc.timePeriodConstraint.id = :timePeriodId")
   List<MonthlyAccessibilityTimePeriodConstraint> findByTimePeriodConstraintId(@Param("timePeriodId") Long timePeriodId);
}
