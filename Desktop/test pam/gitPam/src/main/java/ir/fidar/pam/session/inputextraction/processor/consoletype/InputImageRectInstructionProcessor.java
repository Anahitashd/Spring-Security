...
import ir.fidar.pam.session.inputextraction.processor.extractor.consoletype.InputImageRectExtractor;

public class InputImageRectInstructionProcessor extends AbstractConsoleTypeInstructionProcessor<Void> {
   @Override
   protected InputExtractor<Void> getExtractor() {
      return new InputImageRectExtractor();
   }
   ...
   protected void processExtractedInput(Void input, InputSource source, ConsoleTypeRemoteSessionInputExtraction remoteSessionInputExtraction) {
      remoteSessionInputExtraction.setInputStatus(ConsoleTypeRemoteSessionInputProcessingStatus.IMAGE_RECT);
   }
