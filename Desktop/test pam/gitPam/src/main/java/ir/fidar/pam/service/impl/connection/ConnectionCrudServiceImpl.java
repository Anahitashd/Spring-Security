...
import ir.fidar.core.da.core.query.*;
...
public class ConnectionCrudServiceImpl extends GlobalCommonServiceImpl<Connection> implements ConnectionCrudService {
   ...
   public void create(ConnectionCreateDto connectionCreateDto) throws Exception {
      if (existsByName(connectionCreateDto.getName()))
         throw new ResourceAlreadyExistsException(new ConnectionNameAlreadyExistsException());
      if (existsByHostInfo(connectionCreateDto.getType(), connectionCreateDto.getIpAddress(), connectionCreateDto.getPort()))
         throw new ResourceAlreadyExistsException(new ConnectionHostInfoAlreadyExists());
      checkAVScannerServerReachability(connectionCreateDto);
      ...
      try {
         ...
         SshConnectionCreateDto sshConnectionCreateDto;
         RdpConnectionCreateDto rdpConnectionCreateDto;
         VncConnectionCreateDto vncConnectionCreateDto;
         TelnetConnectionCreateDto telnetConnectionCreateDto;
         ...
         if (connectionCreateDto.getAccessibilityTimePeriodConstraint() != null)
            setAccessibilityTimePeriodConstraintForConnection(connectionCreateDto.getAccessibilityTimePeriodConstraint(), connection);
         if (connectionCreateDto.getSessionInputConstraints() != null && !connectionCreateDto.getSessionInputConstraints().isEmpty())
            setSessionInputConstraintsToConnection(connectionCreateDto.getSessionInputConstraints(), connection);
         if (connectionCreateDto.getBanners() != null && !connectionCreateDto.getBanners().isEmpty())
            setBannersToConnection(connectionCreateDto.getBanners(), connection);
         if (connectionCreateDto.getCredentials() != null && !connectionCreateDto.getCredentials().isEmpty()) {
            ...
            for (CredentialCreateDto credentialCreateDto : connectionCreateDto.getCredentials()) {
               ...
               validateCredentialSupportByConnection(credentialCreateDto.getType(), connection.getType());
               connection.addCredential(convertCredentialCreateDtoToCredential(credentialCreateDto, connection));
               credentialLabels.add(credentialCreateDto.getLabel());
            }
         }
         if (connectionCreateDto.getTransparentPort() != null && this.connectionRepository
                 .existsByTransparentPort(connectionCreateDto.getTransparentPort().intValue()))
         ...
         switch (connectionCreateDto.getType()) {
            case SSH:
               ...
               sshConnectionCreateDto = (SshConnectionCreateDto)connectionCreateDto;
               sshConnection.setConnection(connection);
               copySshSpecificParametersToConnection(sshConnection, sshConnectionCreateDto);
               this.sshConnectionCrudRepository.save(sshConnection);
               break;
            case RDP:
               rdpConnection = new RdpConnection();
               rdpConnectionCreateDto = (RdpConnectionCreateDto)connectionCreateDto;
               rdpConnection.setConnection(connection);
               copyRdpSpecificParametersToConnection(rdpConnection, rdpConnectionCreateDto);
               this.rdpConnectionCrudRepository.save(rdpConnection);
               break;
            case VNC:
               vncConnection = new VncConnection();
               vncConnectionCreateDto = (VncConnectionCreateDto)connectionCreateDto;
               vncConnection.setConnection(connection);
               copyVncSpecificParametersToConnection(vncConnection, vncConnectionCreateDto);
               this.vncConnectionCrudRepository.save(vncConnection);
               break;
            case TELNET:
               telnetConnection = new TelnetConnection();
               telnetConnectionCreateDto = (TelnetConnectionCreateDto)connectionCreateDto;
               telnetConnection.setConnection(connection);
               copyTelnetSpecificParametersToConnection(telnetConnection, telnetConnectionCreateDto);
   ...
   private void setSessionInputConstraintsToConnection(List<SessionInputConstraintViolationHandlerCreateDto> sessionInputConstraintCreateDtoList, Connection connection) {
      ...
      for (SessionInputConstraintViolationHandlerCreateDto sessionInputConstraintHandlerCreateDto : sessionInputConstraintCreateDtoList) {
         SessionInputConstraint sessionInputConstraint = this.sessionInputConstraintRepository.findOneByRegex(sessionInputConstraintHandlerCreateDto.getConstraintRegex());
   ...
   private void checkCaptureRulesAndRemoveIfRequired(Connection connection) {
      JpaQuery<CaptureRule> fetchCaptureRulesSetOverConnectionIndividuallyQuery = (new JpaQueryBuilder()).from(CaptureRule.class, "cr").join("connections", "c").fetch().build();
      List<CaptureRule> captureRules = this.genericJpaQueryBasedReadRepository.findAll((Query)fetchCaptureRulesSetOverConnectionIndividuallyQuery, false);
      for (CaptureRule captureRule : captureRules) {
         captureRule.removeConnection(connection);
         if (captureRule.getConnections().isEmpty()) {
            NativeQuery checkIfCaptureRuleHasAnyConnectionGroup = (new NativeQueryBuilder()).checkExistence().from(ConnectionGroup.class, "cg").joinM2M("tb_capture_rule_connection_group", "crcg", CaptureRule.class, "cr").leftOn("id", "connection_group_id").rightOn("capture_rule_id", "id").joinWhere(QueryAndFilterUtils.idFilter(Long.valueOf(captureRule.getId().longValue()))).build();
